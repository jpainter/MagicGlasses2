  ...
  moduleServer(
    ...
    function(input, output, session) {
          ...
          withProgress(
            ...
            value = 0,
            {
              data.mad = mad_outliers(
                ...
                as_tsibble(key = key_vars(d), index = index_var(d))
            }
          )
      ...
      outlier.summary.cols = reactive({
        ...
        mad_col_index = which(
          grepl('mad', names(d)) & !grepl('not_mad', names(d))
        )
        seasonal_col_index = which(grepl('seasonal', names(d)))

        if (length(mad_col_index) > 0) {
          mad_cols = names(d)[mad_col_index]
        }
        if (length(seasonal_col_index) > 0) {
          seasonal_cols = names(d)[seasonal_col_index]
        }
