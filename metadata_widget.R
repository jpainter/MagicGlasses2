...
# Download  meta data to excel file ####
  output$downloadInfo <- downloadHandler(
    ...
    content = function( file ) {
      ...
      writeDataTable( wb, sheet1, systemInfo() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet2, meta_variables() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet3, orgUnitLevels() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE  )
      writeDataTable( wb, sheet4, orgUnits()|> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars() , rowNames = FALSE )
      writeDataTable( wb, sheet5, dataElementDictionary() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet6, indicatorDictionary() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet7, dataSets.() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars() , rowNames = FALSE )
      writeDataTable( wb, sheet8, categories() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet9, dataElementGroups() |> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars(), rowNames = FALSE )
      writeDataTable( wb, sheet10, ousTree()|> as.data.table() |>  clean_invalid_characters()   |>   truncate_df_chars() , rowNames = FALSE )
