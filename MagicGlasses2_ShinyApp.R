...
source(paste0(mg2, "adjustedCorrectedIncidence.R"))
source(paste0(mg2, "metadata_functions.R"))
...
readFile = function(filename) {
  ...
  if (tolower(ext) == "fst") {
    ...
    stop(
      ...
      ext,
      ". Only .fst and .rds files are supported."
    )
  }
}
...
as.yearmonth = function(date.string, fmt = "%B%Y") {
  zoo::as.yearmon(date.string, fmt) %>% yearmonth
}
...
Month_Year = function(x) {
  yearmonth(zoo::as.yearmon(x, "%Y%m"))
}
...
ui <- fluidPage(
  ...
  navlistPanel(
    ...
    tabPanel(
      ...
      p(
        "- Understand which data are available ",
        span("(Metadata)", style = "color:blue")
      ),
      ...
      p(
        ...
        span("(Reporting)", style = "color:blue")
      ),
      p("- Scan for outliers ", span("(Outliers)", style = "color:blue")),
      p(
        "- Evaluate trends and estimate intervention effectiveness ",
