    ...
    function(
      ...
      formula.request = reactive({
        ...
        if (login() & request()) {
          ...
          cat('\n saving original download (raw) as', saveAs)
          ...
          # saveRDS( d1 , saveAs , compress = FALSE )
        ...
        withProgress(
          ...
          value = 0,
          {
            data.mad = mad_outliers(d1, .threshold = 50) %>%
              as_tsibble(key = key_vars(d1), index = index_var(d1))
          }
        )
        ...
        withProgress(
          ...
          value = 0,
          {
            # data1.seasonal = seasonal_outliers( data.mad , .threshold = 50)

            data1.seasonal = detect_seasonal_outliers(
              data.mad,
              exclude = mad5,
              # value_col = original,
              # deviation = 3,
        ...
        cat('\n saving prepared data as', saveAs)
        saveRDS(data1.seasonal, saveAs, compress = TRUE)

        removeModal()

        cat('\n** Formula Request Finished')
      })
